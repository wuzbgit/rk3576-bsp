...
#define PROC_DIR        "bluetooth-sprd/sleep"
...
int  bluesleep_init(void)
{
	...
	bluetooth_dir = proc_mkdir("bluetooth-sprd", NULL);
	if (bluetooth_dir == NULL) {
		pr_info("Unable to create /proc/bluetooth-sprd directory");
		remove_proc_entry("bluetooth-sprd", 0);
		return -ENOMEM;
	}
	sleep_dir = proc_mkdir("sleep", bluetooth_dir);
	if (sleep_dir == NULL) {
		pr_info("Unable to create /proc/%s directory",
							PROC_DIR);
		remove_proc_entry("bluetooth-sprd", 0);
		return -ENOMEM;
	}
