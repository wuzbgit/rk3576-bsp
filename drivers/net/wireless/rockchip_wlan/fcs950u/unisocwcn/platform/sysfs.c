			  ...
			  void *response, size_t *response_len)
{
	...
	ret = sprdwcn_bus_list_alloc(WCN_AT_TX, &head, &tail, &num);
	...
	ret = sprdwcn_bus_push_list(WCN_AT_TX, head, tail, num);
	if (ret)
...
static int wcn_set_loglevel(void)
{
	...
	if (atomic_read(&sysfs_info.set_mask)
		...
		data = atomic_read(&sysfs_info.set_mask);
		data &= (unsigned int)(~(WCN_SYSFS_LOGLEVEL_SET_BIT));
		atomic_set(&sysfs_info.set_mask,data);		
...
#else
EXPORT_SYMBOL(init_wcn_sysfs);
EXPORT_SYMBOL(exit_wcn_sysfs);
#endif
