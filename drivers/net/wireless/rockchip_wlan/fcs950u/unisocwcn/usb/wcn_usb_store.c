...
#define chn2ep_table_size()\
	(sizeof(chn2ep_table) / sizeof((chn2ep_table)[0]))
...
int wcn_usb_chn_thread_deinit(void)
{
	...
	int i;
	for (i = 0; i < table_size; i++) {
		work_data = wcn_usb_store_get_channel_info(i);
		if (!work_data)
			continue;
		wcn_usb_info("%s stop chn%d thread begin!!\n", __func__, i);
		work_data->exit_flag = 1;
		if(work_data->wcn_usb_thread) {
			complete(&work_data->callback_complete);
			kthread_stop(work_data->wcn_usb_thread);
			work_data->wcn_usb_thread = NULL;
			wcn_usb_info("%s stop chn%d thread success!!\n", __func__, i);
		}
	}
	return 0;
}
