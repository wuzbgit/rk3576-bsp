...
void wcn_usb_ep_stop(struct wcn_usb_ep *ep)
{
	...
	mutex_lock(&ep->submit_stop_lock);
	usb_kill_anchored_urbs(&ep->submitted);
	mutex_unlock(&ep->submit_stop_lock); /* TODO: we should destory muetx while remove*/
...
int wcn_usb_packet_is_freed(struct wcn_usb_packet *packet)
{
	...
	if (atomic_read(&(&packet->urb->kref)->refcount.refs) != 1)
#else
		...
		void *pdata, gfp_t mem_flags)
{
	...
	wcn_usb_debug("%s be called, ep_addr = %x\n", __func__, packet->ep->numEp);
	packet->pdata = pdata;
				...
				const struct usb_device_id *id)
{
	...
	if ((interface_plug_base + interface_num) == interface_2_plug) {
		if(sprdwcn_bus_get_carddump_status()) {
			wcn_usb_info("%s marlin in carddump status!\n", __func__);
			return 0;
		}
...
/*
* return 0 -> resume ok, ready to submit
* return 1 -> in suspend status, wait for resume, not ready to submit
*/
bool wcn_usb_get_suspend_resume_flag(void)
{
	int suspend_flag, resume_flag;

	suspend_flag = atomic_read(&wcn_usb_suspend_flag);
	resume_flag = atomic_read(&wcn_usb_resume_flag);
	if(suspend_flag >= 0 && resume_flag < 0)
		return 1;
	else
		return 0;
...
{
	return usb_deregister(&wcn_usb_io_driver);
}
