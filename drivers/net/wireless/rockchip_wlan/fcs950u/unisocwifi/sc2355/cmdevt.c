...
#if defined(DRV_POWER_DOWN) || defined(DRV_RESET_SELF)
			 ...
			 unsigned int timeout, u8 *rbuf, u16 *rlen)
{
	...
	if (ret != -1) {
		...
		wl_err("ctx_id %d [%s]rsp timeout, printk\n",
...
void sprdwl_wfd_mib_cnt(struct sprdwl_vif *vif, u8 *data, u16 len)
{
	...
	wl_info("%s, %d, drop=%u,%u,%u,%u, frame=%u, clear=%u, mib=%u\n",
		__func__, __LINE__,
