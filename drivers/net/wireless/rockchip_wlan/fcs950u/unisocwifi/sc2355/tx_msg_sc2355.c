...
void sprdwl_flush_data_txlist(struct sprdwl_tx_msg *tx_msg)
{
	enum sprdwl_mode mode;
...
RETRY:
	...
	*/
	...
	if (intf->fw_awake == 0 && sprdwl_debug_level >= L_INFO) {
	...
	if (need_polling && wait_times < TX_MAX_POLLING) {
...
bool is_vowifi_pkt(struct sk_buff *skb, bool *b_cmd_path)
{
	...
	wl_debug("Dual vowifi: skb->mark 0x%x, mark 0x%x\n", skb->mark, mark);
	switch (mark) {
