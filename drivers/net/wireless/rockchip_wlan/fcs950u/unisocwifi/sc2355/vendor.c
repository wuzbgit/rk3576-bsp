...
#define WLAN_CIPHER_SUITE_BIP_GMAC_256 0x000FAC12
#endif
...
int sprdwl_report_acs_lte_event(struct sprdwl_vif *vif)
{
		...
		reply = cfg80211_vendor_event_alloc(wiphy, &vif->wdev,
#endif
        			payload, SPRDWL_ACS_LTE_EVENT_INDEX, GFP_KERNEL);
        if (!reply) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
				nla_strscpy(sae_para.passphrase, pos, sae_para.passphrase_len + 1);
#else
				nla_strlcpy(sae_para.passphrase, pos, sae_para.passphrase_len + 1);
#endif
