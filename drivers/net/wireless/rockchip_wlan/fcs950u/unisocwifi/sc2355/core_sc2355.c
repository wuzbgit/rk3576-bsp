...
int sprdwl_chip_power_on(struct sprdwl_intf *intf)
{
	...
	if (sprdwl_intf_init(priv, intf)) {
		atomic_sub(1, &intf->power_cnt);
		return -ENODEV;
	}
out:
	/*UNISOC: Modify for bug 1994570 @{ */
...
{
	if (sprd_core_get_hwintf_mode() == SPRDWL_HW_SC2355_USB)
		return;
...
{
	enum wcn_hard_intf_type wcn_hw_type;
...
{
	struct resource *res;
	unsigned long int ipa_qos_remap = 0;
	unsigned char temp;
	...
	ipa_qos_remap = (unsigned long int)devm_ioremap(sprdwl_dev, res ->start, resource_size(res));
#else
	ipa_qos_remap = (unsigned long int)devm_ioremap_nocache(sprdwl_dev, res ->start, resource_size(res));
#endif
	wl_debug("ipa_qos_remap=0x%lx\n", ipa_qos_remap);
...
static int sprdwl_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		wl_err("%s Power on wcn failed: %d\n", __func__, ret);
...
#if 0
static void sprdwl_shutdown(struct platform_device *pdev)
{
	struct sprdwl_intf *intf = platform_get_drvdata(pdev);
	struct sprdwl_priv *priv = intf->priv;
